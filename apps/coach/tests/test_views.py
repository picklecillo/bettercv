...
class CoachChatTests(TestCase):
    ...
    def test_no_coach_session_returns_error(self):
        with patch("apps.coach.views.get_coach_service", return_value=_fake()):
...
class CoachParseTests(TestCase):
    ...
    def _post_pdf(self, fake, pdf_content=b"pdf bytes", extra_post=None):
        ...
        with patch("apps.coach.views.get_coach_service", return_value=fake):
            with _patch("apps.coach.views.extract_text_from_pdf", return_value="Extracted CV text"):
    ...
    def test_pdf_takes_priority_over_pasted_text(self):
        ...
        pdf_file.name = "cv.pdf"
        with patch("apps.coach.views.get_coach_service", return_value=fake):
            with _patch("apps.coach.views.extract_text_from_pdf", return_value="PDF text") as mock_extract:
    ...
    def test_unreadable_pdf_returns_error(self):
        ...
        pdf_file.name = "cv.pdf"
        with _patch("apps.coach.views.extract_text_from_pdf", side_effect=PdfExtractionError("Unreadable PDF.")):
