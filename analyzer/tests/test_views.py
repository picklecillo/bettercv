...
class AnalyzeViewTests(TestCase):
    ...
    def test_valid_post_returns_sse_container(self):
        ...
        self.assertIn(b"/analyze/stream/", response.content)

    def test_valid_post_stores_data_in_session(self):
        self.client.post("/analyze/", {"resume_text": "my resume", "jd_text": "some job"})
        session_values = [
    ...
    def test_pdf_upload_takes_priority_over_text(self):
        ...
        session_values = [v for v in self.client.session.values() if isinstance(v, dict)]
...
class StreamViewTests(TestCase):
    ...
    def test_missing_key_returns_400(self):
        response = self.client.get("/analyze/stream/")
        self.assertEqual(response.status_code, 400)

    def test_unknown_key_returns_400(self):
        response = self.client.get("/analyze/stream/?key=nonexistent")
        self.assertEqual(response.status_code, 400)
